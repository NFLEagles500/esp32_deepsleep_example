...
#lower CPU freq
print(f'CPU is running at: {machine.freq()}')
#machine.freq(60000000)
...
if resetCause == 'DEEPSLEEP_RESET':
    ...
    while True:
        try:
            ...
            print(f'trying ntptime again... Exception: {e}')
            utime.sleep(1)
    while True:
        try:
            ...
            print(f'Error from urequests, {e}, trying again...')
            utime.sleep(1)
...
currDateTime = f'{utime.localtime()[0]}/{utime.localtime()[1]}/{utime.localtime()[2]} {utime.localtime()[3] - 7:02d}:{utime.localtime()[4]:02d}:{utime.localtime()[5]:02d}'
with open('log.txt','a') as file:
        file.write(f'{currDateTime} Reset Cause: {resetCause}, going to deepsleep\n')
